...
bool Win32RawInputSource::OpenDevices()
{
	...
	Console.WriteLn("(Win32RawInputSource) Found %u keyboards and %zu mice", m_num_keyboards, m_mice.size());
		...
		for (MouseState& state : m_mice)
		{
			...
			while (button_mask != 0)
			{
				...
				InputManager::InvokeEvents(
					InputManager::MakePointerButtonKey(mouse_index, button_number), static_cast<float>(button_pressed));
